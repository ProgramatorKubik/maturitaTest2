...
class Prihlaska extends Nette\Application\UI\Form{
     ...
     public function __construct($parent, $name) {
        ...
        $this->addRadioList('Vek', 'Vek', array('5 let', '10 let', '15 let'))
                ->setRequired(TRUE);
        ...
        $this->addText('pocet', 'pocet hodin v družině')
                ->setRequired(TRUE)
                ->addRule(\Nette\Forms\Form::INTEGER, 'Neni cislo!');

        $this->addSelect('Sport', 'Mimo školní sport', array('Fotbal', 'Tennis', 'Flordball'))
                ->setRequired(TRUE);
